...
const group: Group = {
  ...
  { page: 'agent-inventory-and-registration', slug: 'agent-inventory-and-registration', title: L('Agent Inventory & Registration','智能体清单与登记'), summary: L('Every other control takes a list of agents as an input and degrades silently without one; make registration the way an agent obtains its credentials, version the (agent, capability, data-scope) triple rather than the agent, and reconcile declared against observed so shadow agents and orphaned credentials become two queries instead of a blind spot.','其他每一项控制都把智能体名单当输入，而名单缺失时它们会静默降级；把登记做成智能体取得凭据的必经之路、给（智能体, 能力, 数据范围）三元组而不是给智能体做版本，并把声明的与观测到的对账，让影子智能体与无主凭据变成两条查询而不是一个盲区。') },
  ],
};
