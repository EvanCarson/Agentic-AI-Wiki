...
const group: Group = {
  ...
  entries: [
    ...
    { page: 'credit-assignment-in-multi-agent-systems', slug: 'credit-assignment-in-multi-agent-systems', title: L('Credit Assignment: Which Agent Do You Change?','功劳分配：你到底该改哪个智能体？'), summary: L('A run-level score contains no gradient, so per-agent judges, ablation and counterfactual replay answer three different questions — local quality, marginal contribution, and specific causation — and reporting the cheapest as though it were the most specific is how a team spends a quarter tuning the worker that did exactly what it was told.','运行级别的分数里没有梯度，于是逐智能体评审、消融与反事实回放回答的是三个不同的问题——局部质量、边际贡献、具体因果——而把最便宜的那个当成最具体的那个来汇报，正是一个团队花一个季度去调那个完全照吩咐办事的工人的原因。') },
  ],
};
