...
const group: Group = {
  ...
  entries: [
    ...
    { page: 'benchmark-contamination', slug: 'benchmark-contamination', title: L('Benchmark Contamination &amp; Leakage','基准污染与泄漏'), summary: L('Contamination belongs to the (model, benchmark, date) triple, not to the benchmark: verbatim vs solution vs indirect leakage, why agent benchmarks leak the environment and not just the answer, four detection tests you can run from outside, and the rule that public scores screen while only post-cutoff data decides.','污染属于（模型，基准，日期）这个三元组，而不属于基准：逐字泄漏、解法泄漏与间接泄漏之分，为何智能体基准泄漏的是环境而不只是答案，四个从外部就能跑的检测，以及"公开分数用来筛选、只有截止日之后的数据才用来决定"这条规则。') },
  ],
};
