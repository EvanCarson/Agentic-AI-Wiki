...
const entry: ChangelogEntry = {
  ...
  items: [
    ...
    L(
      'The new check found real defects on its first runs, which is the point of it — seven labels across six diagrams, none of them visible to anyone reviewing on a Mac. The machine the checks run on renders the same webfont three to eight percent wider than macOS does, and by this site’s own traffic figures roughly seven readers in ten are on Windows or Linux. A caption tuned until it just fits on the author’s screen is therefore genuinely clipped for most people who see it, which is what had happened: a caption overlapping its neighbour in the pgvector architecture diagram, and five labels crossing the edge of their drawing, on the posts about rerankers, sandbox billing, an eval-harness disclosure timeline, generative UI, and eval frameworks. The seventh was published by the scheduled agent on the morning this check went in and was already live when the check caught it, which is the case the check exists for. All seven now sit inside their bounds with room to spare on either platform.',
      '这项检查在最初几次运行中就找出了真实缺陷，而这正是它的意义所在——六张示意图上的七处标签，在 Mac 上复核的人一处也看不见。运行检查的那台机器渲染同一款网页字体时，会比 macOS 宽出百分之三到八；而按本站自己的访问数据，大约每十位读者中有七位使用 Windows 或 Linux。因此一条被调到"刚好放得下"的说明文字，对大多数看到它的人其实是被裁切的——事实正是如此：pgvector 架构图里有一条说明压住了相邻的说明，另有五处标签越出了画面边界，分别在讲重排序模型、沙箱计费、评测平台漏洞披露时间线、生成式 UI 与评测框架的那几篇文章中。第七处出自定时智能体在这项检查落地当天早上发布的内容，被检查发现时已经上线——而这正是这项检查存在的意义。现在这七处在两个平台上都稳稳落在边界之内，且留有余量。',
    ),
  ],
};
