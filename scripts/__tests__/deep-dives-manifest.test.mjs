// Validates every Deep-Dive group file under src/content/deep-dives/groups/
...
const groupsDir = fileURLToPath(new URL('../../src/content/deep-dives/groups/', import.meta.url));
const fragEnDir = fileURLToPath(new URL('../../src/content/deep-dives/en/', import.meta.url));
const fragZhDir = fileURLToPath(new URL('../../src/content/deep-dives/zh/', import.meta.url));
const files = readdirSync(groupsDir).filter(f => f.endsWith('.ts')).sort();

test('deep-dives: at least one group file exists', () => {
  assert.ok(files.length > 0, `no group files under ${groupsDir}`);
});

test('deep-dives: filenames are <kebab-key>.ts', () => {
  const re = /^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.ts$/;
  for (const f of files) assert.match(f, re, `bad group filename: ${f}`);
});

test('deep-dives: every group file is well-shaped and entries match HTML fragments', async () => {
  const keys = new Set();
