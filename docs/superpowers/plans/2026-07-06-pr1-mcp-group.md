...
For links to Concepts pages, use `/concepts/<slug>` (no group segment) — Concepts are ungrouped in URLs. For Operations pages, use `/operations/<group-key>/<slug>` — group keys are `safety-and-security`, `evaluation-and-observability`, `governance-compliance`, `economics-roi`, `agentops`. For Playbooks, use `/playbooks/<group-key>/<slug>` — group keys are `agent-ux-and-human-interaction`, `coding-and-computer-use-agents`, `domain-playbooks`, `voice-realtime-agents`. Verify each target URL exists by inspecting a rendered existing xref before shipping (`grep 'href="/operations/' src/content/deep-dives/en/*.html` for examples).
